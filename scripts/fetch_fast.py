...
def fetch_fear_greed():
    ...
    Note: CNN's endpoint requires a start-date suffix in the URL path (it returns
    a history from that date to today) - hitting it with no date is unreliable.
    We ask for the last ~10 days and just take the most recent point.
    """
    ...
    try:
        ...
        # Preferred: CNN exposes a live "fear_and_greed" block with the current score
        fg = data.get("fear_and_greed")
        if fg and fg.get("score") is not None:
            return {"score": round(float(fg["score"]), 1), "rating": fg.get("rating")}
 
        # Fallback: take the most recent point from the historical series
