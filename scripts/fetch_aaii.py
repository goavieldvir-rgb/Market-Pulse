...
for free, publicly, on aaii.com (no login needed to view the headline numbers).
There's no official API, so this does a best-effort scrape of the public page.

AAII only updates this once a week (Thursdays), so this only needs to run
once a day - re-running more often just re-confirms the same number until
Thursday. Designed to fail soft: if AAII changes their page layout, we keep
whatever was last successfully written rather than crashing the workflow.
"""
import json
import os
import re
...
def extract_pct(html, label):
    """Look for a pattern like 'Bullish: 31.0%' or 'Bullish ... 31.0 %'."""
    patterns = [
...
def main():
    try:
        ...
        sys.exit(0)  # fail soft, keep previous file
    ...
    if bullish is None or bearish is None:
        print("WARN: could not parse AAII numbers from page - site layout may have changed", file=sys.stderr)
        sys.exit(0)
